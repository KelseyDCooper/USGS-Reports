...
park_n <- ifelse(which(park == park.list$park.code),paste(park.list$park.name), park)
park_name <- as.character(park_n)
total_entries <- table1$iNaturalist_Entry_count
total_entries <- sum(table1[, 'iNaturalist_Entry_count'])
taxon_categories <- "TAXON CAT Placeholder"
...
new_invasives <- nrow(table3[which(table1$Match.NPSpecies=="Yes"), ])
...
__Table 1.__ Research Grade species recorded during the `r total_entries` NPS Centennial Bioblitz and whether the species matches an existing record in its NPSpecies list or reflects a taxonomic change (Table 2), a previously unrecorded species in the NPSpecies database (Table 3), or a species that falls into one of the following taxa: `r unique_taxa` (Table 4).
